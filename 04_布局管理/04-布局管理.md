...
- [笔记](#笔记)
    ...
    - [PyQt5中的布局管理](#pyqt5中的布局管理)
        ...
        - [QBoxLayout 框布局](#qboxlayout-框布局)
            - [QHBoxLayout 水平布局](#qhboxlayout-水平布局)
            - [QVBoxLayout 垂直布局](#qvboxlayout-垂直布局)
...
addStretch(int stretch=0) : 在布局管理器中增加一个可伸缩的控件(QSpaceItem), 0为最小值, 兵器将stretch作为伸缩添加到布局末尾. stretch参数表示均分的比例, 默认值为0  
